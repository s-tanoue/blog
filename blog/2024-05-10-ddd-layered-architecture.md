...
flowchart LR
    ...
    subgraph restaurant["🍽️ レストラン"]
        Waiter["🏪 店員<br/>Interface層"]
        Chef["📋 調理リーダー<br/>Application層"]
        Recipe["👨‍🍳 レシピ・ルール<br/>Domain層"]
        Kitchen["🔧 調理器具・食材<br/>Infrastructure層"]
    end
    ...
    Chef -->|"調理指示"| Kitchen
    Kitchen -->|"料理"| Waiter
    Waiter -->|"料理提供"| Customer

    style Recipe fill:#ff9,stroke:#333,stroke-width:2px
    style Customer fill:#e1f5fe,stroke:#333
```
...
まず、4つのレイヤーがどのように連携するか全体像を見てみましょう。

```mermaid
flowchart TB
    subgraph interface["🌐 Interface層"]
        direction LR
        HTTP["REST API<br/>Controller"]
        CLI["CLI<br/>コマンド"]
    ...
    subgraph application["⚙️ Application層"]
        direction LR
        UseCase1["ユースケース<br/>注文作成"]
        UseCase2["ユースケース<br/>注文キャンセル"]
    end

    subgraph domain["💎 Domain層（ビジネスの核心）"]
        direction LR
        Entity["エンティティ<br/>Order, User"]
        ValueObject["値オブジェクト<br/>Money, Address"]
        DomainService["ドメインサービス<br/>在庫チェック"]
        Repository["リポジトリ<br/>インターフェース"]
    end

    subgraph infra["🔧 Infrastructure層"]
        direction LR
        DB["データベース<br/>PostgreSQL"]
        External["外部API<br/>決済, 通知"]
    ...
    style interface fill:#cce5ff,stroke:#004085
    style application fill:#d4edda,stroke:#155724
    style infra fill:#f8d7da,stroke:#721c24
```

**ポイント**: Domain層が中心にあり、他のどの層にも依存しません。これがDDDの最も重要な原則です。
...
具体的なコードを見ていきましょう。まず、注文作成時のデータの流れを図で確認します。

```mermaid
sequenceDiagram
    participant Client as 🌐 クライアント
    participant Controller as Interface<br/>OrderController
    participant Service as Application<br/>OrderService
    participant Order as Domain<br/>Order
...
```mermaid
flowchart TB
    subgraph Interface["🌐 Interface層（窓口）"]
        I1["HTTPリクエストの受け取り"]
        I2["レスポンスの返却"]
        I3["エラーをステータスコードに変換"]
    end

    subgraph Application["⚙️ Application層（流れ）"]
        A1["処理の順番を決める"]
        A2["トランザクション管理"]
        A3["Domain層とInfra層を組み合わせる"]
    end
    ...
    subgraph Infrastructure["🔧 Infrastructure層（道具）"]
        IN1["データベース接続"]
        IN2["外部API呼び出し"]
        IN3["ファイル読み書き"]
    end

    Interface -->|"DTOを渡す"| Application
    Application -->|"ビジネスルールを呼ぶ"| Domain
    Infrastructure -.->|"インターフェースを実装"| Domain

    style Domain fill:#fff3cd,stroke:#856404,stroke-width:3px
    style D4 fill:#ffc107,stroke:#856404
```

この図の重要なポイントは、**Infrastructure層がDomain層のインターフェースを「実装」している**点です。Domain層はインターフェースを定義するだけで、具体的な実装には依存しません。
...
- Interface と Application は内側のレイヤーを使う

この「依存性逆転の原則（DIP）」により、データベースや外部APIを変更してもビジネスロジックに影響しません。
